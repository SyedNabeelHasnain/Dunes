    ...
    protected function buildIndexMarkdown(): string
    {
        ...
        if ($safariTours->count() > 0) {
            foreach ($safariTours as $tour) {
                if ($tour->slug === 'dune-buggy-rental-dubai') continue;
    ...
    protected function buildFullMarkdown(): string
    {
        ...
        if ($tours->count() > 0) {
            foreach ($tours as $idx => $tour) {
                ...
                if ($tour->pickup_time) $md .= "- **Pickup Schedule**: {$tour->pickup_time}\n";
                if ($tour->min_age) $md .= "- **Minimum Age**: {$tour->min_age} Years\n";
                if ($tour->group_size) $md .= "- **Group Size**: {$tour->group_size}\n";
                if ($tour->languages) $md .= "- **Guide Languages**: {$tour->languages}\n";
