...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <h1 class="text-base font-black text-slate-900">New Blog Post Details</h1>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div class="lg:col-span-8 space-y-6">
                        <div class="bg-slate-50/80 rounded-2xl border border-slate-200/80 p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="excerpt" id="excerpt" rows="2" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs text-slate-800 placeholder-slate-400 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all outline-hidden" placeholder="Brief summary displayed on listings" required>{{ old('excerpt') }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="content" id="content" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-sm text-slate-800 wysiwyg-editor focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all outline-hidden" rows="12" placeholder="Write article here..." required>{{ old('content') }}</textarea>
                        ...
                        <div class="bg-slate-50/80 rounded-2xl border border-slate-200/80 p-5 space-y-4">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="author_name" id="author_name" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-xs text-slate-800 focus:border-primary outline-hidden" value="{{ old('author_name', 'Dunes Discovery') }}" placeholder="Dunes Discovery">
                                </div>
                                <div>
                                    ...
                                    <input type="text" name="author_title" id="author_title" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-xs text-slate-800 focus:border-primary outline-hidden" value="{{ old('author_title', 'Dubai Tourism Expert') }}" placeholder="Dubai Tourism Expert">
                                </div>
                                <div class="sm:col-span-2">
                                    ...
                                    <textarea name="author_bio" id="author_bio" rows="2" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-primary outline-hidden" placeholder="Biographical details shown at post footer">{{ old('author_bio') }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-slate-50/80 rounded-2xl border border-slate-200/80 p-5 space-y-3">
                            ...
                            <textarea name="ai_summary" id="ai_summary" rows="2" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-primary outline-hidden" placeholder="A brief bulleted AI-generated summary shown in an highlight box at the top of the post">{{ old('ai_summary') }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-4 space-y-6">
                        <div class="bg-slate-50/80 rounded-2xl border border-slate-200/80 p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="published_at" id="published_at" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-primary outline-hidden" value="{{ old('published_at') }}">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="read_time" id="read_time" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-primary outline-hidden" value="{{ old('read_time', 5) }}" required>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="schema_type" id="schema_type" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-primary outline-hidden">
                                    <option value="BlogPosting" {{ old('schema_type', 'BlogPosting') === 'BlogPosting' ? 'selected' : '' }}>BlogPosting (Default)</option>
                                    ...
                                    <option value="TravelAdvisory" {{ old('schema_type') === 'TravelAdvisory' ? 'selected' : '' }}>TravelAdvisory</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="bg-slate-50/80 rounded-2xl border border-slate-200/80 p-5 space-y-3">
                            ...
                            <div>
                                ...
                                <input type="text" name="tags[]" id="tags_input" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-primary outline-hidden" value="{{ is_array(old('tags')) ? implode(', ', old('tags')) : old('tags') }}" placeholder="dubai, safari, adventure">
                        ...
                        <div class="bg-slate-50/80 rounded-2xl border border-slate-200/80 p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="meta_title" id="meta_title" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-primary outline-hidden" value="{{ old('meta_title') }}">
                            </div>
                            <div>
                                <label for="meta_desc" class="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-1.5">Meta Description</label>
                                <textarea name="meta_desc" id="meta_desc" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-primary outline-hidden" rows="3">{{ old('meta_desc') }}</textarea>
                            </div>
                            <div>
                                <label for="meta_keywords" class="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-1.5">Meta Keywords</label>
                                <input type="text" name="meta_keywords" id="meta_keywords" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-primary outline-hidden" value="{{ old('meta_keywords') }}">
                            </div>
                            <div>
                                ...
                                <input type="url" name="canonical_url" id="canonical_url" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-primary outline-hidden" value="{{ old('canonical_url') }}" placeholder="https://...">
                            </div>
                        </div>
                        ...
                        <div class="bg-slate-50/80 rounded-2xl border border-slate-200/80 p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="featured_image_caption" id="featured_image_caption" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-primary outline-hidden" placeholder="e.g. Dubai Red Dunes Safari">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-12 flex justify-end pt-4 border-t border-slate-100">
                        <button type="submit" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-primary hover:bg-primary-dark text-white font-bold text-sm shadow-xs transition-all">
                            <i class="bi bi-check-lg"></i> Publish Blog Post
                        </button>
                    </div>
                </div>
            </form>
