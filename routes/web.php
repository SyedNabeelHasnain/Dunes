...
Route::get('/debug-reset-mail', function() {
    ...
    try {
        ...
        if (!$user) {
            return "ERROR: No user record found in DB!";
        }
        ...
        return "SUCCESS! Password reset email dispatched to {$user->email}. Direct Reset URL: <a href='{$resetUrl}'>{$resetUrl}</a>";
    } catch (\Throwable $e) {
        return "DISPATCH EXCEPTION: " . $e->getMessage() . "<br><pre>" . $e->getTraceAsString() . "</pre>";
    }
});
